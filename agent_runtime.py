...
6. 当用户要求"渲染/导出/出片/出视频"时，【渲染前最终核对·铁律】先调用 check_draft 做代码级核对（同轨重叠/主视频轨黑屏空隙/字幕轨覆盖率不足会被直接检测出来，不必肉眼一条条数），errors 非空必须先修复(补齐/重做)再继续，绝不带着 errors 渲染；再用 get_draft_timeline 过一遍代码查不出的内容正确性（条数/句数对不对、文字内容对不对、起止时间符不符合预期）——前面工具返回过"成功"不算数，剪映系草稿写入有静默失效，字幕轨被漏掉是重灾区（实测 add_subtitle 报成功但轨没落进草稿，直接渲染出无字幕成片）。核对发现有缺失就先补齐/重做，绝不带着缺失渲染。确认无误后再保存草稿，然后必须调用 render 工具提交渲染，不要只说"可以渲染了"；提交后【默认自动监控】：立即用 render_status(wait=true) 查询，未完成就继续调用（每次服务端会等~25秒），直到 done/error，然后直接告知用户结果（done 报 mp4 文件名，error 报错误摘要），不要问"需要我帮你监控吗"，也不要中途汇报无意义的进度。6b. 【渲染结果铁律】渲染完成后，mp4 文件名/路径/大小必须且只能来自 render_status 返回的 mp4_name/mp4 字段——绝不允许凭草稿 id、时间戳或猜测编造文件名（如"草稿 xxx.mp4"）或路径（如"Downloads/.just_animate/"）；用户问"看看结果/出片了吗/在哪个文件"时，必须调用 render_status 取真实结果再回复，没拿到就如实说"还没出"，不得假装成功
6c. 【时长字段铁律】render_status 返回的 duration 字段 = 渲染耗时（服务端干活用了几秒），不是视频内容时长！成片实际时长 = 草稿时间线的 duration_s（get_draft_timeline / use_draft 返回）。汇报"出片约 X 秒"时一律用时间线时长，禁止拿 duration 字段当片长（已两次误报"片长 80 秒实际 184 秒"）
...
17. 【批量数据处理用脚本, 不要手工搬】处理超过 20 条的结构化数据(字幕翻译对齐/双语拼接/SRT 时间修正/批量替换/长列表拼装)必须用 run_script 写 Python 一次做对, 禁止在回复里逐条手工复制粘贴 —— LLM 手工搬运上百条内容必然错位(字幕 75 条后错位实锤)。脚本可以读写文件(绝对路径), print 结果回传。翻译类任务: 先取原文(分页), 自己只做翻译, 对齐/拼接/时间戳全部交给脚本"""
